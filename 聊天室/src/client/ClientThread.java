...
public class ClientThread extends Thread {
        ...
        try {
            while (Data.s.isConnected()) {
                ...
                if (type.equals("login")) {
                    ...
                    if (msg.getType() == 1) ChatFrame.appendText(ChatFrame.myArea, msg.getMessage());
                    ...
                    ChatFrame.appendText(ChatFrame.myArea, "【文件消息】对方拒绝接收，文件发送失败！\n");
                }
    ...
    private void sendFile(Response response) {
        ...
        try {
            Socket s = new Socket(sendFile.getTargetIp(), sendFile.getTargetPort());//套接字连接
            BufferedInputStream bis = new BufferedInputStream(new FileInputStream(sendFile.getSrcName()));//文件读入
            ...
            bos.flush();
            synchronized (this) {
                ChatFrame.appendText(ChatFrame.myArea, "【文件消息】文件发送完毕!\n");
            }
            bis.close();
            bos.close();
            s.close();
    ...
    private void receiveFile(Response response) {
        ...
        try {
            ServerSocket serverSocket = new ServerSocket(sendFile.getTargetPort());
            Socket s = serverSocket.accept(); //接收
            BufferedInputStream bis = new BufferedInputStream(s.getInputStream());//缓冲读
            BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(sendFile.getTargetName()));//缓冲写出
            ...
            bos.flush();
            synchronized (this) {
                ChatFrame.appendText(ChatFrame.myArea, "【文件消息】文件接收完毕!存放在[" + sendFile.getTargetName()+"]\n");
            }
            bis.close();
            bos.close();
            s.close();
        ...
        try {
            ...
            if (select == JOptionPane.YES_OPTION) {
                ...
                if (result == JFileChooser.APPROVE_OPTION){
                    ...
                    res.setType("agree");
                    ChatFrame.appendText(ChatFrame.myArea, "【文件消息】您已同意接收来自 " + fromName +" 的文件，正在接收文件 ...\n");
                }
                else {
                    res.setType("refuse");
                    ChatFrame.appendText(ChatFrame.myArea, "【文件消息】您已拒绝接收来自 " + fromName +" 的文件!\n");
                }
            }
            else {
                res.setType("refuse");
